...
def likelihood(signalParams, backgroundParams, f, cosThetaK, cosThetaL, phi, time, mass, qSS, qOS, wSS, wOS, normB, normBBar):
    ...
    b = individualTimeDependent(normB, 1, cosThetaL, cosThetaK, phi, time, mass, *signalParams)
    bBar = individualTimeDependent(normBBar, -1, cosThetaL, cosThetaK, phi, time, mass, *signalParams)
    pdf = (
             weightB *
             (f*b + (1-f)*background) +
             weightBBar *
    ...
    for generationName in ['theory', 'alternative']:
        for q2_range in range(3):
            for luminosity in range(3):
                ...
                for tagging in ['untagged', 'badTagging', 'goodTagging']:
